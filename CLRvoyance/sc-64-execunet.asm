...
INDEX_OLEAUT32_SAFEARRAYGETLBOUND       equ 9 * POINTERSIZE + STACKSPACE
INDEX_OLEAUT32_SAFEARRAYGETUBOUND       equ 10 * POINTERSIZE + STACKSPACE

APIFUNCTIONCOUNT 				equ 11
...
PMETHODPARAMS 		  equ 0x150
SAFEARRAYGETLBOUND    equ 0x158
SAFEARRAYGETUBOUND    equ 0x160
LUPPER    			  equ 0x168
LLOWER    			  equ 0x170
...
execunet:
	...
	mov rsi, [gs:0x28]
	add rsi, SAFEARRAYGETLBOUND
	LOOKUP_API OLEAUT32DLL, OLEAUT32_SAFEARRAYGETLBOUND, rsi

	mov rsi, [gs:0x28]
	add rsi, SAFEARRAYGETUBOUND
	LOOKUP_API OLEAUT32DLL, OLEAUT32_SAFEARRAYGETUBOUND, rsi
    ...
    %ifdef APPDOMAIN_NAME
        mov rax, [gs:0x28]
        add rax, CORRUNTIMEHOST
        mov rax, [rax]
        mov rax, [rax]
        lea rdx, [rel APPDOMAIN_VALUE]
        xor r8, r8
        mov r9, [gs:0x28]
        add r9, APPDOMAINTHUNK
        mov rcx, [gs:0x28]
        add rcx, CORRUNTIMEHOST
        mov rcx, [rcx]
        call [rax+0x60]
	...
	; mMethodInfo->GetParameters(&pMethodParams)
	mov rax, [gs:0x28]
	add rax, METHODINFO
	mov rax, [rax]
	mov rax, [rax]
	mov rdx, [gs:0x28]
	add rdx, PMETHODPARAMS
	mov rcx, [gs:0x28]
	add rcx, METHODINFO
	mov rcx, [rcx]
	call [rax+0x90]
	...
	mov rdx, 1
	mov rcx, [gs:0x28]
	add rcx, PMETHODPARAMS
	mov rcx, [rcx]
	mov rdi, [gs:0x28]
	call [rdi+SAFEARRAYGETLBOUND]
	test eax,eax
	jnz done
	...
	; pMethodArgs = _SafeArrayCreateVector(VT_VARIANT, 0, lArgs);
	mov r8, rax
	xor rdx,rdx
	mov rcx, 0x0c
	mov rsi, [gs:0x28]
